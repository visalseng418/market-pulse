...
- **Frontend** (Phase 10 — not started): React + Vite + TypeScript, Tailwind CSS, shadcn/ui, Zustand, Socket.io client, Recharts
...
GET    /api/market/prices       (auth)
GET    /api/alerts              (auth)
POST   /api/alerts              (auth)
DELETE /api/alerts/:id          (auth)
GET    /api/indicators/:symbol  (auth)
...
- ⬜ Phase 11: Deploy (Railway/Render + Neon + Upstash + Vercel)

### Phase 10 — Frontend Pages

```
/login, /register
/dashboard       — live price ticker, all assets
/watchlist       — saved assets with live prices
/alerts          — manage price alerts
/indicators/:symbol — RSI, MACD, SMA charts
```

Features: WebSocket live updates, candlestick/line charts, price flash animation, "last updated" counter, Zustand global state, JWT in memory (not localStorage)
